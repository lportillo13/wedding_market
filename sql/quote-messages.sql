...
grant select, insert, update, delete on public.quote_messages to service_role;

create index if not exists quote_messages_quote_created_idx
  on public.quote_messages using btree (quote_id, created_at asc);
...
create policy quote_messages_client_select
...
create policy quote_messages_vendor_select
  on public.quote_messages
  for select
  to authenticated
  using (
...
create policy quote_messages_client_insert
  on public.quote_messages
  for insert
  to authenticated
  with check (
...
create policy quote_messages_vendor_insert
  on public.quote_messages
  for insert
  to authenticated
  with check (
