#!/usr/bin/env node
/**
 * scripts/generate-supabase-schema-pg-no-tpl.mjs
 * Usage:
 *   node scripts/generate-supabase-schema-pg-no-tpl.mjs [outputPath]
 *
 * Env:
...
// --- load .env.local then .env (no deps) ---
...
// Build SQL strings without template literals
const sqlTables = [
...
// queries
const { rows: tables } = await client.query(sqlTables, schemaList);
const { rows: columns } = await client.query(sqlColumns, schemaList);
const { rows: constraints } = await client.query(sqlConstraints, schemaList);
const { rows: indexes } = await client.query(sqlIndexes, schemaList);
...
for (const t of tables) {
  ...
  map[t.schema][t.name] = { kind: t.kind, comment: t.comment ?? null, columns: [], constraints: [], indexes: [] };
}
